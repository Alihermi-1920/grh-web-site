...
const ChefAttendance = () => {
    ...
    fetch("http://localhost:5000/api/presence")
      ...
      .then((data) => {
        if (!Array.isArray(data)) {
          ...
          // Filtrer les enregistrements pour ne garder que ceux des employés du chef
          const employeeIds = employees.map(emp => emp._id);
          const filteredRecords = data.filter(record =>
            record.employeeId && employeeIds.includes(record.employeeId._id)
          );

          setAttendanceRecords(filteredRecords);
          console.log(`Récupération de ${filteredRecords.length} enregistrements de présence pour les employés du chef`);
        }
  ...
  useEffect(() => {
    ...
    // Récupérer uniquement les employés qui appartiennent au chef connecté
    fetch(`http://localhost:5000/api/employees/chef/${user._id}`)
      .then((res) => {
        if (!res.ok) {
          throw new Error(`Erreur HTTP: ${res.status} - Erreur lors de la récupération des employés`);
        }
        return res.json();
      })
      .then((data) => {
      ...
      .catch((err) => {
        ...
        // Fallback: si l'endpoint spécifique échoue, essayer de récupérer tous les employés et filtrer
        console.log("Tentative de récupération de tous les employés comme fallback...");
        fetch("http://localhost:5000/api/employees")
          .then((res) => res.json())
          .then((allEmployees) => {
            // Filtrer pour ne garder que les employés qui ont ce chef
            const filteredEmployees = allEmployees.filter(emp => emp.chefId === user._id);
            setEmployees(filteredEmployees);

            // Extraire les départements uniques
            const uniqueDepartments = [...new Set(filteredEmployees.map(emp => emp.department))];
            setDepartments(uniqueDepartments);
          })
          .catch((fallbackErr) => {
            console.error("Erreur lors de la récupération de tous les employés :", fallbackErr);
          });
      });
  }, [user]);
  ...
  const filteredEmployees = employees.filter(employee => {
    ...
    return matchesSearch && hasNoRecord;
  });
  ...
  const generateReport = (period = 'day') => {
    ...
    fetch(`http://localhost:5000/api/reports/daily?date=${format(selectedDate, "yyyy-MM-dd")}&period=${period}`)
      ...
      .then((data) => {
        ...
        // Filtrer les détails pour ne garder que les employés du chef
        const employeeIds = employees.map(emp => emp._id);
        const filteredDetails = data.details.filter(record =>
          record.employeeId && employeeIds.includes(record.employeeId._id)
        );

        // Recalculer les statistiques basées sur les employés filtrés
        const present = filteredDetails.filter(record => record.status === 'present').length;
        const late = filteredDetails.filter(record => record.status === 'late').length;
        const ropo = filteredDetails.filter(record => record.status === 'ropo').length;
        const absent = filteredDetails.filter(record => record.status === 'absent').length;

        // Créer un nouveau rapport avec les données filtrées
        ...
        console.log(`Statistiques filtrées: Présents: ${present}, Retards: ${late}, Congés: ${ropo}, Absents: ${absent}`);
        ...
        console.log(`Détails filtrés: ${filteredDetails.length} enregistrements`);
    ...
    <Box>
      ...
      <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Typography variant="h5" sx={{ fontWeight: 'bold' }}>
          Système de présence de mon équipe
        </Typography>
