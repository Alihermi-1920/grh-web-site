...
const EmployeeTaskChat = () => {
  ...
  // Fetch tasks assigned to the current user (employee)
  useEffect(() => {
    const fetchTasks = async () => {
      setLoading(true);
      try {
        // Fetch tasks where the current user is the assignedEmployee
        const response = await fetch(`http://localhost:5000/api/tasks?assignedEmployee=${user._id}`);
        if (!response.ok) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      formData.append("receiverId", selectedTask.assignedBy._id);
                      ...
                      <ListItemText
                        ...
                        secondary={
                          <Typography variant="body2" color="text.secondary" noWrap>
                            {task.assignedBy ? 
                              `Chef: ${task.assignedBy.firstName} ${task.assignedBy.lastName}` : 
                              "Non assigné"}
                          </Typography>
                        }
                      />
          ...
          {selectedTask ? (
              ...
              >
                ...
                <Box sx={{ mt: 1 }}>
                  <Typography variant="body2">
                    <strong>Description:</strong> {selectedTask.details}
                  </Typography>
                </Box>
              </Box>
            ...
            >
              ...
              <Typography variant="body2" color="text.secondary" align="center">
                Choisissez une tâche dans la liste pour voir les messages et communiquer avec votre chef
              </Typography>
            </Box>
          )}
        </Box>
...
export default EmployeeTaskChat;
